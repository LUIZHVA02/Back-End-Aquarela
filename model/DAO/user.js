...
const selectFeed = async (id) => {
  try {
    let sql = `
      ...
      FROM tbl_produto AS tp
      LEFT JOIN tbl_categoria_produto AS tcp 
          ON tp.id_produto = tcp.id_produto 
          AND tcp.categoria_produto_status = true
      LEFT JOIN tbl_curtida_produto AS cp 
          ON tp.id_produto = cp.id_produto 
          AND cp.id_usuario = ${id}
      LEFT JOIN tbl_produto_favorito AS pf 
          ON tp.id_produto = pf.id_produto 
          AND pf.id_usuario = ${id}
      JOIN tbl_usuario AS tu 
          ON tp.id_usuario = tu.id_usuario 
          AND tu.usuario_status = true
      ...
      SELECT
          ...
          CAST(1 AS DECIMAL) AS preferencia
      FROM tbl_postagem AS tp
      LEFT JOIN tbl_categoria_postagem AS tcp 
          ON tp.id_postagem = tcp.id_postagem 
          AND tcp.categoria_postagem_status = true
      LEFT JOIN tbl_curtida_postagem AS cp 
          ON tp.id_postagem = cp.id_postagem 
          AND cp.id_usuario = ${id}
      LEFT JOIN tbl_postagem_favorita AS pf 
          ON tp.id_postagem = pf.id_postagem 
          AND pf.id_usuario = ${id}
      JOIN tbl_usuario AS tu 
          ON tp.id_usuario = tu.id_usuario 
          AND tu.usuario_status = true
